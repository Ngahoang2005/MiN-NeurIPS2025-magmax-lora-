...
class PiNoise(torch.nn.Linear):
    def __init__(self, in_dim, out_dim, hidden_dim=384):
        super(torch.nn.Linear, self).__init__() # Giữ nguyên super của bác
        self.bias = None
        ...
        nn.init.xavier_uniform_(self.w_down); nn.init.xavier_uniform_(self.w_up)

        # [CHỈNH]: Đổi ModuleList thành Linear đơn (Sequential)
        self.mu = nn.Linear(self.hidden_dim, self.hidden_dim).to(device)
        self.sigmma = nn.Linear(self.hidden_dim, self.hidden_dim).to(device)
        torch.nn.init.constant_(self.mu.weight, 0.); torch.nn.init.constant_(self.mu.bias, 0.)
        torch.nn.init.constant_(self.sigmma.weight, 0.); torch.nn.init.constant_(self.sigmma.bias, 0.)

        # MagMax: Lưu trọng số ban đầu
    ...
    def after_task_training(self):
        ...
        t_sig = {k: self.sigmma.state_dict()[k].detach().cpu() - self.base_sig_sd[k].cpu() for k in self.base_sig_sd.keys()}
        self.history_tau_mu.append(t_mu); self.history_tau_sig.append(t_sig)
        
        def merge(base, hist, layer):
            merged = {}
            for k in base.keys():
                s = torch.stack([h[k] for h in hist], dim=0)
                best = torch.gather(s, 0, torch.abs(s).argmax(0, keepdim=True)).squeeze(0)
                merged[k] = base[k].to(layer.weight.device) + best.to(layer.weight.device)
            return merged
        self.mu.load_state_dict(merge(self.base_mu_sd, self.history_tau_mu, self.mu))
        self.sigmma.load_state_dict(merge(self.base_sig_sd, self.history_tau_sig, self.sigmma))
