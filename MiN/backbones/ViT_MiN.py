...
def emr_merge_tensors(tensor_list):
    ...
    if current_scale == 0: return unified_param 
...
class PiNoise(torch.nn.Linear):
    ...
    def unfreeze_noise(self):
        # [MODIFIED] Hot-Init: Khởi tạo nhiễu nhỏ để Orthogonal Loss có tác dụng
        for param in self.mu[-1].parameters():
            param.requires_grad = True
            if param.dim() > 1: torch.nn.init.normal_(param, std=0.001)
    ...
    def forward(self, x):
        ...
        if self.active_task_idx >= 0:
            ...
            # Universal Branch (Dùng trọng số đã Merge)
