...
class MinNet(object):
    ...
    def increment_train(self, data_manger):
        ...
        # [TỐI ƯU]: Loader cho Train backbone (Batch nhỏ để học kỹ)
        train_loader = DataLoader(train_set, batch_size=self.batch_size, shuffle=True,
                                  num_workers=self.num_workers)
        
        # [TỐI ƯU]: Loader cho Test
        test_loader = DataLoader(test_set, batch_size=self.batch_size, shuffle=False,
                                 num_workers=self.num_workers)
        self.test_loader = test_loader
        ...
        # -------------------------------------------------------------------
        # BƯỚC 1: FIT CLASSIFIER LẦN 1 (RLS)
        # -------------------------------------------------------------------
        self._network.update_fc(self.increment)
        
        # [SPEED UP]: Dùng batch to (512) cho RLS chạy nhanh gấp 5 lần
        train_loader_fast = DataLoader(train_set, batch_size=512, shuffle=True, num_workers=self.num_workers)
        self.fit_fc(train_loader_fast, test_loader)

        # -------------------------------------------------------------------
        # [NEW FEATURE] DRIFT MONITOR (Kẹp giám sát Task 0)
        # -------------------------------------------------------------------
        print(f"\n[MONITOR] Checking Backbone Stability before Task {self.cur_task} training...")
        ...
        self._network.eval()
        with torch.no_grad():
            # Đo Feature TRƯỚC khi train
            feats_before = self._network.backbone(ref_imgs)
            feats_before = F.normalize(feats_before, p=2, dim=1)
        # -------------------------------------------------------------------

        # -------------------------------------------------------------------
        # BƯỚC 2: TRAIN BACKBONE / NOISE (GPM)
        # -------------------------------------------------------------------
        self._network.update_noise()
        self._clear_gpu()
        
        # Train Backbone (Vẫn dùng batch nhỏ self.batch_size để hội tụ tốt)
        self.run(train_loader) 
        ...
        # -------------------------------------------------------------------
        # [CHECK] TÍNH ĐIỂM DRIFT SAU KHI TRAIN
        # -------------------------------------------------------------------
        self._network.eval()
        with torch.no_grad():
            feats_after = self._network.backbone(ref_imgs)
            feats_after = F.normalize(feats_after, p=2, dim=1)
        
        drift_score = (feats_before * feats_after).sum(dim=1).mean().item()
        print(f"--> [MONITOR] Stability Score: {drift_score:.4f}")
        
        if drift_score < 0.90:
            print("!!! CẢNH BÁO: Backbone bị biến dạng (Drift) quá mạnh! Acc Class cũ sẽ giảm sâu.")
            print("!!! KHUYẾN NGHỊ: Giảm Learning Rate hoặc Freeze backbone.")
        # -------------------------------------------------------------------

        # -------------------------------------------------------------------
        # BƯỚC 3: RE-FIT (RLS LẦN 2 - QUAN TRỌNG NHẤT)
        # -------------------------------------------------------------------
        train_set_noaug = data_manger.get_task_data(source="train_no_aug", class_list=train_list)
        train_set_noaug.labels = self.cat2order(train_set_noaug.labels, data_manger)
        ...
        del train_set_noaug, test_set, t0_set
        self._clear_gpu()
    ...
    def fit_fc(self, train_loader, test_loader):
        ...
        # [SỬA LỖI]: Bỏ vòng lặp epoch! RLS chỉ cần nhìn dữ liệu 1 lần.
        # Chúng ta thu thập toàn bộ dữ liệu (One-Shot) hoặc chạy 1 pass qua loader
        
        print(f"--> [Fast RLS] Fitting Analytical Classifier (Task {self.cur_task})...")
        
        # Cách 1: One-Shot (Nhanh nhất nếu RAM đủ - Khuyên dùng cho CIFAR100)
        all_inputs = []
        all_targets = []
        ...
        Y_all = torch.cat(all_targets)
        
        # Chuyển One-hot
        Y_all = torch.nn.functional.one_hot(Y_all, num_classes=self._network.known_class)
        
        # Gọi fit đúng 1 lần
        self._network.fit(X_all, Y_all)

        self.logger.info(f"Task {self.cur_task} --> Analytical Classifier Updated (One-Shot)!")
        self._clear_gpu()
    def re_fit(self, train_loader, test_loader):
        self._network.eval()
        self._network.to(self.device)
