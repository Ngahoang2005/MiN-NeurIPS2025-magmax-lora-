...
class MinNet(object):
    def __init__(self, args, loger):
        ...
        # RESTORE GỐC
        self.init_epochs, self.init_lr, self.init_weight_decay, self.init_batch_size = \
            args["init_epochs"], args["init_lr"], args["init_weight_decay"], args["init_batch_size"]
        self.lr, self.batch_size, self.weight_decay, self.epochs = \
            args["lr"], args["batch_size"], args["weight_decay"], args["epochs"]
        self.init_class, self.increment, self.buffer_size, self.buffer_batch, self.gamma, self.fit_epoch = \
            args["init_class"], args["increment"], args["buffer_size"], args["buffer_batch"], args["gamma"], args["fit_epochs"]

        self.known_class, self.cur_task = 0, -1
        self.total_acc, self.class_acc, self.task_acc = [], [], []
    ...
    def init_train(self, data_manger):
        ...
        # Task 0: Chạy đúng Fit gốc
        self.fit_fc(DataLoader(train_set, batch_size=self.buffer_batch, shuffle=True, num_workers=self.num_workers), self.test_loader)
        ...
        self.re_fit(DataLoader(train_set_no_aug, batch_size=self.buffer_batch, shuffle=True, num_workers=self.num_workers), self.test_loader)
    ...
    def fit_fc(self, train_loader, test_loader):
        ...
        for _ in prog_bar:
            for _, inputs, targets in train_loader:
                # [FIX CRASH]: one_hot tự do như bản gốc
                y_oh = F.one_hot(targets.to(self.device))
                self._network.fit(inputs, y_oh)
            # DPCR: Nén sau mỗi epoch để tiết kiệm
            self._network.compress_stats(); self._clear_gpu()
            prog_bar.set_description(f"Task {self.cur_task} --> Updating Classifier")
        self._network.solve_with_drift()
    ...
    def re_fit(self, train_loader, test_loader):
        ...
        for _, inputs, targets in tqdm(train_loader, desc="Refitting"):
            y_oh = F.one_hot(targets.to(self.device))
            self._network.fit(inputs, y_oh)
        self._network.compress_stats()
        self._network.solve_with_drift(P_drift=P, boundary=boundary)
        self._clear_gpu()
    ...
    def run(self, train_loader):
        ...
        scheduler = get_scheduler(self.args['scheduler_type'], optimizer, (self.init_epochs if self.cur_task==0 else self.epochs))
        self._network.train()
        prog_bar = tqdm(range(self.init_epochs if self.cur_task==0 else self.epochs))
        for epoch in prog_bar:
            ...
            for _, inputs, targets in train_loader:
                ...
                total += len(targets)
            scheduler.step()
            prog_bar.set_description(f"Task {self.cur_task} | Ep {epoch+1} | Acc {100.*correct/total:.2f}")
        self._clear_gpu()
