...
class MinNet(object):
    ...
    def _snapshot_local_weights(self):
        """Lưu toàn bộ trọng số thực tế đang tham gia vào forward pass"""
        self.old_noise_weights = []
        for block in self._network.backbone.noise_maker:
            self.old_noise_weights.append({
                # Bắt đúng tên biến đang dùng trong hàm forward của PiNoise
                'mu': block.fc_mu.weight.detach().clone(),
                'sigma': block.fc_rho.weight.detach().clone()
            })
    ...
    def run(self, train_loader):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if self.cur_task > 0 and len(self.old_noise_weights) > 0:
                    for idx, block in enumerate(self._network.backbone.noise_maker):
                        ...
                        # Phạt fc_rho
                        w_sigma_curr = block.fc_rho.weight.float()
                        w_sigma_old = old_dict['sigma'].to(w_sigma_curr.device).float()
                        dot_sigma = torch.matmul(w_sigma_curr, w_sigma_old.t())
                        batch_ortho += torch.sum(dot_sigma.pow(2)) / w_sigma_curr.shape[0]
                        
                    # Cộng dồn Thành phần 3 vào tổng Loss
            ...
            if self.cur_task > 0:
                info = "T {} | Ep {} | L {:.3f} (CE {:.3f} | KL {:.1f} | Ortho {:.4f}) | Acc {:.2f}".format(
                    self.cur_task, epoch + 1, 
                    losses / len(train_loader), 
                    ce_losses / len(train_loader),
                    kl_losses / len(train_loader),
                    ortho_losses / len(train_loader),
                    train_acc
                )
            else:
