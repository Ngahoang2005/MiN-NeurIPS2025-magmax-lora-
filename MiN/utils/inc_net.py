...
class MiNbaseNet(nn.Module):
    ...
    def fit(self, X: torch.Tensor, Y: torch.Tensor) -> None:
        ...
        Hàm Fit tối ưu: 
        1. KHÔNG chuẩn hóa.
        2. KHÔNG tính lại Mean (Chỉ tiêu thụ Centroid đã có).
        """
        self.backbone.eval()
        ...
        # 2. Sinh mẫu giả (Dùng Centroid Global đã tính trước)
        if self.prev_known_class > 0:
            unique_new = torch.argmax(Y_cpu, dim=1).unique().tolist()
            
            # Lọc danh sách Centroids của các lớp mới có trong batch này
            # (Dùng normalize tạm thời chỉ để tính Cosine chọn Seed giống nhất)
            available_new_means = [self.class_means[c] for c in unique_new if self.class_means[c] is not None]
            
            if len(available_new_means) > 0:
                new_means_mat = torch.stack(available_new_means)
                ...
                for c_old in range(self.prev_known_class):
                    if self.class_means[c_old] is not None:
                        ...
                        # Chọn Seed dựa trên hướng (Cosine) để đảm bảo độ tương đồng
        ...
        # 3. Gộp và RLS Update (Không chuẩn hóa)
        ...
        RHt = H @ self.R.T 
        A = RHt @ H.T 
        A.diagonal().add_(1e-6) # Jitter cho ổn định số học
        
        try:
            K = RHt.T @ torch.linalg.solve(A + torch.eye(A.shape[0], device=self.device), 
                                           torch.eye(A.shape[0], device=self.device))
        except:
            K = self.R @ H.T @ torch.inverse(A + torch.eye(A.shape[0], device=self.device))
